...
4.0.0 upgrades the library to **Spring Data JPA 3.x** with **JDK 17** as the Java baseline and the **Jakarta Persistence** stack.

#### Breaking changes

1. **JDK 17 is required.** The compiler, Gradle toolchain, and CI now target Java 17. JDK 8 / 11 are not supported.
2. **Jakarta Persistence replaces `javax.persistence`.** Custom `Specification` lambdas and Criteria API imports must use `jakarta.persistence.criteria.*`. Spring Data JPA 3.x does not work with `javax.persistence`.
3. **Spring Data JPA 3.x / Spring Boot 3.x.** This release is not compatible with Spring Data JPA 2.x or Spring Boot 2.x.
4. **`PageRequest` constructors are gone** in Spring Data 3. Use `PageRequest.of(page, size)` / `PageRequest.of(page, size, sort)`.
5. **Coordinates:** compile against `jakarta.persistence:jakarta.persistence-api` (3.1) instead of `org.hibernate.javax.persistence:hibernate-jpa-2.1-api`.
6. **Maven Central replaces JCenter.** JCenter/Bintray are shut down; the Bintray publish plugin was removed. Use `mavenCentral()` and `maven-publish`.

Builder APIs (`Specifications.and()` / `or()`, `PredicateBuilder`, `Sorts`) keep the same method names and call patterns.
...
The library is `compileOnly` against Spring Data JPA and Jakarta Persistence. Your application should provide Spring Data JPA 3.x (typically via `spring-boot-starter-data-jpa`).
...
each Repository class should extend **JpaRepository** and **JpaSpecificationExecutor**.
...
Custom predicates must use **Jakarta** Criteria types (`jakarta.persistence.criteria.Path`, `Join`, `JoinType`).
